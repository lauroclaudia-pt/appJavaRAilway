...
public class Vaga {
    ...
    /** Data de publicação específica na Bolsa de Emprego Público — distinta da publicação no website (secção 4.1). */
    @Column(name = "bep_publication_date")
    ...
    private Trabalhador gestor;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "gestor_suplente_id")
    private Trabalhador gestorSuplente;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "manager_cdrh_id")
    private Trabalhador managerCdrh;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "juri_pres_id")
    private Trabalhador juriPresidente;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "juri_ve1_id")
    private Trabalhador juriVogalEfetivo1;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "juri_ve2_id")
    private Trabalhador juriVogalEfetivo2;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "juri_vs1_id")
    private Trabalhador juriVogalSuplente1;
