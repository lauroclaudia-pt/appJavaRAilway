...
public class SecurityConfig {
    ...
    /**
     * O login é feito por email; as autoridades concedidas são a UNIÃO de todas as
     * responsabilidades atribuídas ao trabalhador (ex.: alguém CDRH + JURI ao mesmo
     * tempo recebe ROLE_CDRH e ROLE_JURI).
     */
    @Bean
    public UserDetailsService userDetailsService() {
        return (String email) -> trabalhadorRepository.findByEmailIgnoreCase(email)
                .filter(pt.ipma.recrutamento.domain.Trabalhador::isActive)
                .filter(pt.ipma.recrutamento.domain.Trabalhador::hasLogin)
